...
DFS(Graph G, int start)      广度遍历函数 参数：图G,开始结点下标start 作用：宽度遍历
BFS(Graph G, int start)      深度遍历函数（递归形式）参数：图G,开始结点下标start 作用：深度遍历
Dijkstra(Graph G, int v)     最短路径 - Dijkstra算法 参数：图G、源点v
功能实现函数：
CreateGraph(Graph &G) 创建图功能实现函数 参数：图G  InsertNode 作用：创建图
DFSTraverse(Graph G)  广度遍历功能实现函数 参数：图G 作用：宽度遍历
BFSTraverse(Graph G)  深度遍历功能实现函数 参数：图G 作用：深度遍历
